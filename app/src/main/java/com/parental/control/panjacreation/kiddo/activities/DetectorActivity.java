...
import com.parental.control.panjacreation.kiddo.customview.OverlayView;
import com.parental.control.panjacreation.kiddo.env.ImageUtils;
...
import com.parental.control.panjacreation.kiddo.tracking.MultiBoxTracker;
...
import com.parental.control.panjacreation.kiddo.env.BorderedText;
...
public class DetectorActivity extends CameraActivity implements OnImageAvailableListener {
  ...
  public void onPreviewSizeChosen(final Size size, final int rotation) {
    ...
    faceBmp = Bitmap.createBitmap(Constants.TF_OD_API_INPUT_SIZE, Constants.TF_OD_API_INPUT_SIZE, Config.ARGB_8888);
  ...
  private void onFacesDetected(long currTimestamp, List<Face> faces, boolean add) {
    ...
    for (Face face : faces) {
      ...
      if (boundingBox != null && goodConfidence) {
        ...
        float sx = ((float) Constants.TF_OD_API_INPUT_SIZE) / faceBB.width();
        float sy = ((float) Constants.TF_OD_API_INPUT_SIZE) / faceBB.height();
        Matrix matrix = new Matrix();
        ...
        final List<SimilarityClassifier.Recognition> resultsAux = detector.recognizeImage(faceBmp, add, getApplicationContext());
